...
def rotation_to_heading(rotation):
    """Convert the quaternion rotation to the heading of agent.
       Works only for 4-action agent, namely no looking up and down"""
    theta = np.arctan2(rotation.imag[1], rotation.real)
    heading = theta * 2
    heading = np.array([heading], dtype = float32)
    return heading 
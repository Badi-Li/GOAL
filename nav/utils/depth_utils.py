...
def bin_points(XYZ_cms, map_size, z_bins, xy_resolution):
    ...
    for XYZ_cm in XYZ_cms:
        ...
        X_bin = np.round(XYZ_cm[:, :, 0] / xy_resolution).astype(int32)
        Y_bin = np.round(XYZ_cm[:, :, 1] / xy_resolution).astype(int32)
        Z_bin = np.digitize(XYZ_cm[:, :, 2], bins=z_bins).astype(int32)
        ...
        count = np.bincount(ind.ravel(), isvalid.ravel().astype(int32),
...
def get_point_cloud_from_z_t(Y_t, camera_matrix, device, scale=1):
    ...
    Outputs:
        ...
        Y is positive into the image
        Z is positive up in the image
        XYZ is ...xHxWx3: The point clouds' coordinates corresponding to each pixel in depth image
    """
    ...
    XYZ = torch.stack((X_t, Y_t[:, ::scale, ::scale], Z_t), dim=len(Y_t.shape))
...
def transform_camera_view_t(XYZ, sensor_height, camera_elevation_degree, device):
    """
    Transforms the point cloud into egocentric frame to account for
    camera elevation and angle
    Input:
        XYZ                     : ...x3
        sensor_height           : height of the sensor
